...
class FileUploadAPIView(generics.CreateAPIView):
    ...
    def extract_text_from_pdf(self,uploaded_file):
        ...
        # Convert all_tables to a formatted string
        formatted_tables = "\n\n".join("\n".join(row) for row in all_tables)

        template = f'''Extract Policy Details according you :
            ...
            From the text: {all_tables}, and return 'nan' if any data does not exist.'''

        genai.configure(api_key="AIzaSyAwBHzkyqJ2S7jB72LFyvt9uga_OEipRsk")
        model = genai.GenerativeModel('gemini-pro')
        response = model.generate_content(template.format(formatted_tables=formatted_tables))
        text = response.text
        policy_dict = {}
